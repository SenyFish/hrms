...
public class EmployeeRelationsMenuInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        Menu relations = ensureMenu("员工关系", "/relations", "User", 70, 0L);
        Menu contracts = ensureMenu("签合同", "/relations/contracts", "Document", 71, relations.getId());
        Menu disputes = ensureMenu("处理纠纷", "/relations/disputes", "Avatar", 72, relations.getId());

        attachMenus("ADMIN", relations, contracts, disputes);
        attachMenus("HR", relations, contracts, disputes);
        attachMenus("EMP", relations, contracts, disputes);
    }
