...
如果你希望 MySQL 在首次启动容器时自动导入 SQL，请按照 [`DEPLOY-DOCKER.md`](DEPLOY-DOCKER.md) 中的说明，将 SQL 文件放到 `deploy/sql/` 目录下。如果你想直接使用当前仓库已有的初始化脚本，请在运行 Compose 前将 [`database/init-hrms.sql`](database/init-hrms.sql) 复制到该目录。
